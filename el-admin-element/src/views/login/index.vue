                    ...
                    login(this.form.username, this.form.password).then(response => {
                        ...
                        if (resp.flag) {
                            ...
                            getUserInfo(resp.data.token).then(response => {
                                // console.log('yang', response.data);
                                const respUser = response.data;
                                if (respUser.flag) {
                                    // console.log(respUser.flag);
                                    // 保存token和用户信息
                                    localStorage.setItem('el-admin-token', resp.data.token);
                                    localStorage.setItem('el-admin-user', JSON.stringify(respUser.data));
                                    // 跳转到登录页
                                    this.$router.push('/');
                                } else {
                                    this.$message({
                                        message: '这是一条警告消息！！！' + respUser.message,
                                        type: 'warning',
                                    });
                                }
                            });
